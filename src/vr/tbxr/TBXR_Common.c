...
static void EglInitExtensions()
{
	...
	// Query the driver directly: gl4es is not initialised yet at this point.
...
static bool ovrFramebuffer_Create(
    ...
    for (uint32_t i = 0; i < frameBuffer->TextureSwapChainLength; i++) {
        ...
        // Tell gl4es about the swapchain image so its name maps 1:1 (see gl4es_ext.h).
        gl4es_registerExternalTexture(colorTexture, colorTextureTarget, width, height);
        ...
        if (glRenderbufferStorageMultisampleEXT != NULL &&
            ...
            // Plain framebuffer (no multisampling): all calls go through gl4es.
            GL(glGenRenderbuffers(1, &frameBuffer->DepthBuffers[i]));
            GL(glBindRenderbuffer(GL_RENDERBUFFER, frameBuffer->DepthBuffers[i]));
...
void ovrFramebuffer_Resolve(ovrFramebuffer* frameBuffer) {
    ...
    // glInvalidateFramebuffer is not available through gl4es (optimisation hint only).
...
void TBXR_InitialiseOpenXR()
{
	...
	strcpy(appInfo.applicationName, "TorcsVR");
...
void TBXR_submitFrame()
{
	...
	if (!VR_UseScreenLayer()) {
		...
		// The game tells us which part of eye 0's swapchain image holds the 2D
		// view and how big (in metres) the floating screen should be.
		int rx = 0, ry = 0;
		int rw = gAppState.Renderer.FrameBuffer[0].ColorSwapChain.Width;
		int rh = gAppState.Renderer.FrameBuffer[0].ColorSwapChain.Height;
		float sizeX = 2.4f, sizeY = 1.8f;
		VR_GetScreenLayerRect(&rx, &ry, &rw, &rh, &sizeX, &sizeY);
		...
		quad_layer.subImage.imageRect.offset.x = rx;
		quad_layer.subImage.imageRect.offset.y = ry;
		quad_layer.subImage.imageRect.extent.width = rw;
		quad_layer.subImage.imageRect.extent.height = rh;
		quad_layer.subImage.imageArrayIndex = 0;
		VrGetScreenQuad(&quad_layer.pose, &sizeX, &sizeY);
