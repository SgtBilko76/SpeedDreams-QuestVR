 ...
 * GfuiEventLoop (src/libs/tgfclient/guieventloop.cpp, ANDROID branch).
 *
...
/* Consumer side (guieventloop.cpp). Returns 1 and fills *ev, or 0 when empty. */
int VrPollEvent(VrEvent* ev);

/* Current VR mouse cursor position, in window pixels, y down (gui.cpp uses it
